...
echo "🔐 Step 5/8: Generating self-signed SSL certificate..."
ssh $K8S_HOST << 'EOF'
...
echo "☸️  Step 6/8: Creating Kubernetes resources..."
...
# Step 7: Restart deployment if it exists
echo "⏳ Step 7/8: Restarting deployment..."
if kubectl get deployment sound-recorder -n $NAMESPACE &> /dev/null; then
  kubectl rollout restart deployment/sound-recorder -n $NAMESPACE
  echo "✅ Deployment restart initiated"
else
  echo "ℹ️  New deployment, no restart needed"
fi
echo ""

# Step 8: Wait for deployment
echo "⏳ Step 8/8: Waiting for deployment to be ready..."
